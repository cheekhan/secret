...
export function use12Branches(
  ...
  // 计算每个地支的角度
  const pathAngle = useArcPath(
    SizeInfo.branchesSize[0],
    SizeInfo.branchesSize[1],
    12
  ); // 路径和角度
  ...
  branchesGroup
    .attr(
      "transform",
      (_, i) => `rotate(${i * pathAngle.angle * (180 / Math.PI)})`
    )
    .append("path")
    .attr("d", pathAngle.path) // 绘制路径
  ...
  branchesGroup
    ...
    .attr("transform", `rotate(${(pathAngle.angle * (180 / Math.PI)) / 2 - 8})`)
    .attr("y", 200); // 文本旋转,靠左
  // 绑定寄宫
  branchesGroup
    .append("g")
    .attr(
      "transform",
      `rotate(${(pathAngle.angle * (180 / Math.PI)) / 2 - 18})`
    )
    .selectAll("text")
    .data((d) => getStemFromBranch(d))
    .join("text")
    .attr("fill", Color.BranchStem)
    .text((d) => d)
    .style("font-size", 14)
    .attr("y", (_, i, arr) => {
      // 计算寄宫天干的排列位置
...
/** 创建天盘,需要使用月将和占时进行对齐 */
export function use12Stems(
  container: Selection<SVGSVGElement, any, any, any>,
  branchIndex: number
) {
  // 计算每个地支的角度
  const pathAngle = useArcPath(
    SizeInfo.stemsSize[0],
    SizeInfo.stemsSize[1],
    12
  ); // 路径和角度
  ...
  branchesGroup
    ...
    .append("path")
    .attr("d", pathAngle.path)
    .attr("fill", Color.StemBg) // 绘制路径
  ...
  branchesGroup
    .append("text")
    .text((d) => d)
    .style("font-weight", 600) // 显示地支名称
    .attr("y", SizeInfo.stemsSize[1] + 27)
    .attr("fill", Color.StemColor)
...
export function use12Godes(container: Selection<SVGSVGElement, any, any, any>) {
  ...
  // 设置g元素的transform属性，使其中心在SVG画布的中心
  g.attr("transform", "translate(225,225)");
  const godesGroup = g.selectAll("g").data(godes).join("g");
  ...
  godesGroup
    ...
    .attr("y", SizeInfo.stemsSize[0] + 27)
    .attr("fill", Color.GodColor)
    .attr("text-anchor", "middle"); // 设置文本位置和颜色
  g.rotate = _12Rotate(g, rotateAngle);
  return g;
}
